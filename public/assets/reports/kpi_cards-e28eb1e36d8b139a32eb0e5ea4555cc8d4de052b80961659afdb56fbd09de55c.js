...
function growthBadge(growth, prev) {
  const style = 'font-size:.62rem;font-weight:600;';
  if (growth > 0) return `<span class="badge bg-success-subtle text-success border border-success-subtle" style="${style}">▲${growth}%</span>`;
  if (growth < 0) return `<span class="badge bg-danger-subtle text-danger border border-danger-subtle" style="${style}">▼${Math.abs(growth)}%</span>`;
  if (prev > 0) return `<span class="badge bg-secondary-subtle text-secondary" style="${style}">→ igual</span>`;
  return '';
}
